...
func main() {
	...
	cfg, err := config.LoadConfigWithServiceID("auth")
	if err != nil {
	...
	if err := database.AutoMigrate(&models.User{}, &models.Session{}, &models.VoiceProfile{}); err != nil {
...
func setupRoutes(router *gin.Engine, cfg *config.Config, database *db.Database, log *logger.Logger) {
	...
	api := router.Group("/api/v1")
	{
		...
		userRepo := repository.NewUserRepository(database.DB, log)

		// Инициализация сервисов
		authService := service.NewAuthService(userRepo, cfg, log)

		// Инициализация обработчиков
		authHandlers := handlers.NewAuthHandlers(authService, cfg, log)

		// Настройка маршрутов для аутентификации
		handlers.SetupAuthRoutes(api, authHandlers, cfg, database.DB)

		// Запускаем периодическую очистку устаревших сессий в фоновом режиме
		go func() {
			for {
				time.Sleep(cfg.Auth.SessionsCleanup)
				if err := authService.CleanupExpiredSessions(context.Background()); err != nil {
					log.Error("Error cleaning up expired sessions", zap.Error(err))
				}
			}
		}()
	}
