...
import (
	...
	_ "backend/profile-service/docs"
...
func main() {
	...
	cfg, err := config.LoadConfigWithServiceID("profile")
	if err != nil {
	...
	if err := database.AutoMigrate(&models.VoiceProfile{}); err != nil {
...
func setupRoutes(router *gin.Engine, cfg *config.Config, database *db.Database, log *logger.Logger) {
	...
	api := router.Group("/api/v1")
	{
		...
		profileRepo := repository.NewProfileRepository(database.DB, log)

		// Инициализация сервисов
		profileService := service.NewProfileService(profileRepo, cfg, log)

		// Инициализация обработчиков
		profileHandlers := handlers.NewProfileHandlers(profileService, cfg, log)

		// Настройка маршрутов для профилей
		handlers.SetupProfileRoutes(api, profileHandlers, cfg, database.DB)
	}
}
