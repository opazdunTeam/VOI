<template>
  <div class="flex min-h-screen bg-gray-100 w-full">
    <!-- Левая панель с информацией о продукте -->
    ...
    <div class="w-full md:w-1/2 flex items-center justify-center px-6 py-12">
      <div class="w-full max-w-md">
        <div class="text-center mb-10">
          ...
          <p class="text-gray-600">Войдите, чтобы продолжить работу с VOY</p>
        </div>
        
        <!-- Показываем ошибку только если она связана с входом, а не с автоматической проверкой -->
        <div v-if="loginError" class="mb-6 p-4 bg-red-50 border-l-4 border-red-500 text-red-700 rounded">
          <p>{{ loginError }}</p>
        </div>
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <div class="mb-6">
            <div class="flex justify-between items-center mb-2">
              ...
              <a href="#" class="text-sm text-primary hover:text-indigo-800">
                Забыли пароль?
              </a>
            </div>
            <input
        ...
        <div class="mt-8 text-center">
          <p class="text-gray-600">
            Нет аккаунта? 
            <router-link to="/register" class="text-primary font-medium ml-1 hover:text-indigo-800">
...
const handleSubmit = async () => {
  try {
    ...
    // После успешной авторизации перенаправляем на дашборд
    router.push('/dashboard')
  } catch (error) {
    console.error('Login error:', error)
    loginError.value = authStore.error || 'Ошибка при входе'
  }
}
</script> 